...
##1
f = open('datasets/'+sub_dir+'/triviaqa-train_new.json')
nq = json.load(f)
...
nq_refs = transform("tr_",nq)

#save
f = open('datasets/'+sub_dir+'/tr_ref_train.json', 'w', encoding='utf-8')
...
#a
import json
f = open('datasets/'+sub_dir+'/tr_ref_train.json', encoding='utf-8')
nq_refs = json.load(f)
f.close()
...
f = open('datasets/'+sub_dir+'/traintr3x.json', 'w', encoding='utf-8')
json.dump(global_merged_list,f, ensure_ascii=False,indent=1)
f.close()

## ----------restart

f = open('datasets/'+sub_dir+'/traintr3x.json')
...
f = open('datasets/'+sub_dir+'/triviaqa-dev_new.json')
nq_dev = json.load(f)
f.close()
print_stats(nq_dev)

nqdev_refs = transform("trd_",nq_dev)
nqdev_tagged_testq = get_training_q(nqdev_refs) ## for testing/validation
len(nqdev_tagged_testq)
# 8837

with open('datasets/'+sub_dir+'/trdev_tagged_testq.pickle', 'wb') as handle:
...
with open('datasets/'+sub_dir+'/trdev_pids_untrain.pickle', 'wb') as handle:
    pickle.dump(nqdev_pids_untrain, handle, protocol=pickle.HIGHEST_PROTOCOL)
# with open('datasets/'+sub_dir+'/trdev_pids_untrain.pickle', 'rb') as handle:
...
with open('datasets/'+sub_dir+'/trdev_passages.pickle', 'wb') as handle:
...
with open('datasets/'+sub_dir+'/trdev_tagged_testq.pickle', 'rb') as handle:
...
with open('datasets/'+sub_dir+'/trdev_passages.pickle', 'rb') as handle:
...
def trainOpt(trainTestData,OnlyTestData=None):
    ...
    domain = [
        ...
        {'name': 'var_4', 'type': 'discrete', 'domain': (2,5,9)},
