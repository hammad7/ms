...
f = open('datasets/biencoder-nq-train.json')
nq = json.load(f)
f.close()
...
def transform(prefix,data):
    ...
    for cnt,i in enumerate(data):
        dataset.append({"qid":prefix+str(cnt),"q":i['question'],"pid":list(set([j["passage_id"] for j in i["positive_ctxs"]]))})
    return dataset

nq_refs = transform("n_",nq)

#save
f = open('datasets/nq_ref_train.json', 'w', encoding='utf-8')
...
#load corpus
import pandas as pd
# wiki = pd.read_csv ("datasets/psgs_w100.tsv", sep = '\t',nrows = 3000000) ### nrows = 8000000
wiki = pd.read_csv ("datasets/psgs_w100.tsv", sep = '\t') ### nrows = 8000000

wiki

## extrac all pids of question
...
## validation set
f = open('datasets/biencoder-nq-dev.json')
nq_dev = json.load(f)
f.close()
print_stats(nq_dev)

nqdev_refs = transform("nd_",nq_dev)
nqdev_tagged_testq = get_training_q(nqdev_refs) ## for testing/validation

with open('datasets/nqdev_tagged_testq.pickle', 'wb') as handle:
...
with open('datasets/nqdev_pids_untrain.pickle', 'wb') as handle:
    pickle.dump(nqdev_pids_untrain, handle, protocol=pickle.HIGHEST_PROTOCOL)
with open('datasets/nqdev_pids_untrain.pickle', 'rb') as handle:
...
with open('datasets/nqdev_passages.pickle', 'wb') as handle:
    pickle.dump(nqdev_passages, handle, protocol=pickle.HIGHEST_PROTOCOL)


#########

## convert to tagged doc
def taggedDoc(nq_pids_batch):
	tagged_passage_list = []
	for row in tqdm(nq_pids_batch):
		try:
			doc = wiki.loc[wiki.id==int(row)]
			# print(doc)
			docStr = doc["title"].iloc[0] + " " + doc["text"].iloc[0]  ## append title
...
# def taggedDoc111(nq_pids_batch):
# 	for row in tqdm(nq_pids_batch):
# 		doc = wiki.loc[wiki.id==int(row)]
# 		if isinstance(doc["title"].iloc[0], float) or isinstance(doc["text"].iloc[0], float):
# 			print(doc)
# 			break



#save question passages
f = open('datasets/nq_passages.json', 'w', encoding='utf-8')
json.dump(nq_passages,f, ensure_ascii=False,indent=2)
f.close()

#load question passages
f = open('datasets/nq_passages.json', encoding='utf-8')
temp = json.load(f)
f.close()
nq_passages = []
for row in tqdm(temp):
	nq_passages.append(TaggedDocument(words=row[0],tags=row[1]))

len(nq_passages)

# for index, row in tqdm(wiki.iterrows()):
#     if(str(row["id"]) not in nq_pids): ## not Random
#         non_nq_pids.append(row["id"])
#         if len(non_nq_pids)>=_corpus*len(nq_pids):   ### total ~xx15l passages, without POOL 7****
#             print(index)
#             break
...
## without pool
# nq_passages = taggedDoc(list(nq_pids))



#save question passages
f = open('datasets/nonnq_passages3x.json', 'w', encoding='utf-8')
json.dump(nonnq_passages,f, ensure_ascii=False,indent=2)
f.close()
# temp = nonnq_passages

#load question passages
f = open('datasets/nonnq_passages3x.json', encoding='utf-8')
temp = json.load(f)
f.close()

nonnq_passages = []
for row in tqdm(temp):
    nonnq_passages.append(TaggedDocument(words=row[0],tags=row[1]))

len(nonnq_passages)
#1628243

# question, pid (retrieval)
# title-passage, pid  (indexing)
...
len(tagged_sentence_list)

## merge
global_merged_list = tagged_sentence_list + nq_passages + nonnq_passages

## temp checkpoint for data
with open('datasets/train3x.pickle', 'wb') as handle:
    pickle.dump(global_merged_list, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
# f = open('datasets/global_merged_list.json', 'w', encoding='utf-8')
# json.dump(global_merged_list,f, ensure_ascii=False,indent=2)
# f.close()

best = trainOpt(None)
# dbow,sum,remove tagIds from passage-later vectors, wikidata moretodo-test model training

# asd = blackboxAgent([[0.001,0.01]])
# par_={"alpha":0.01,"min_alpha":0.001,"steps":100}

# testModel(asd,random.sample(range(len(tagged_sentence_list)),400),tagged_sentence_list,123,False,par_)
...
def blackboxAgent(params):  ## DEBUGGER METHOD
    ...
    results = testModel(model,random.sample(range(len(tagged_sentence_list)),400),tagged_sentence_list,123,False,e_parametrs)
    ...
    resultsValid = testModel(model,random.sample(range(len(nqdev_tagged_testq)),400),nqdev_tagged_testq,123,False,e_parametrs)
...
def trainOpt(trainTestData,OnlyTestData=None):
    ...
    domain = [
        ...
        {'name': 'var_1', 'type': 'continuous', 'domain': (0.0045,0.017)},
        {'name': 'var_2', 'type': 'continuous', 'domain': (0.01,0.017)},
        {'name': 'var_3', 'type': 'discrete', 'domain': (2,)},
        {'name': 'var_4', 'type': 'discrete', 'domain': (5,)},
        {'name': 'var_5', 'type': 'discrete', 'domain': (500,650,800)},
...
# len([1 for i in nq if len(i["negative_ctxs"])>0])
# len([1 for i in nq if len(i["hard_negative_ctxs"])>0])

# len([1 for i in corpus.values() if i["title"]!=""])


sum([1 for row in wiki if row["title"]=="" ])

# import csv
# fields = []
# wiki = []
  
# with open("datasets/psgs_w100.tsv", 'r') as csvfile:
