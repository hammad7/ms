...
##1
f = open('datasets/'+sub_dir+'/webquestions-train.json')
nq = json.load(f)
...
nq_refs = transform("wq_",nq)

#save
f = open('datasets/'+sub_dir+'/wq_ref_train.json', 'w', encoding='utf-8')
...
#a
import json
f = open('datasets/'+sub_dir+'/wq_ref_train.json', encoding='utf-8')
nq_refs = json.load(f)
f.close()
...
f = open('datasets/'+sub_dir+'/trainwq3x.json', 'w', encoding='utf-8')
json.dump(global_merged_list,f, ensure_ascii=False,indent=1)
f.close()

## ----------restart

f = open('datasets/'+sub_dir+'/trainwq3x.json')
...
f = open('datasets/'+sub_dir+'/webquestions-dev.json')
nq_dev = json.load(f)
f.close()
print_stats(nq_dev)
# Distinct Q 278
...
nqdev_refs = transform("wqd_",nq_dev)
nqdev_tagged_testq = get_training_q(nqdev_refs) ## for testing/validation
len(nqdev_tagged_testq)
# 278

with open('datasets/'+sub_dir+'/wqdev_tagged_testq.pickle', 'wb') as handle:
...
with open('datasets/'+sub_dir+'/wqdev_pids_untrain.pickle', 'wb') as handle:
...
with open('datasets/'+sub_dir+'/wqdev_passages.pickle', 'wb') as handle:
...
with open('datasets/'+sub_dir+'/wqdev_tagged_testq.pickle', 'rb') as handle:
...
with open('datasets/'+sub_dir+'/wqdev_passages.pickle', 'rb') as handle:
...
def blackboxAgent(params):  ## DEBUGGER METHOD
    ...
    model = Doc2Vec(window=wind, vector_size = siz_ ,alpha=alpha,min_alpha=min_alpha, epochs=1, 
                    ...
                    dm=dm_, dbow_words=1,dm_mean=1,callbacks=[epochlogger])
    model.build_vocab(global_merged_list)
    start = time.time()
    _ep = 10
    model.train(global_merged_list,total_examples = model.corpus_count,epochs=_ep, report_delay=2,callbacks=[epochlogger] )
    end = time.time()
    # if self.debuglevel:
    ...
    results = testModel(model,random.sample(range(len(tagged_sentence_list)),len(tagged_sentence_list)),tagged_sentence_list,123,False,e_parametrs)
    ...
    resultsValid = testModel(model,random.sample(range(len(nqdev_tagged_testq)),len(nqdev_tagged_testq)),nqdev_tagged_testq,123,False,e_parametrs)
...
def trainOpt(trainTestData,OnlyTestData=None):
    ...
    domain = [
        ...
        {'name': 'var_5', 'type': 'discrete', 'domain': (300,)},
